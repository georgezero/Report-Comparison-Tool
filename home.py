...
from utils.LMStudio_Agent import extract_LMStudio
...
# LMSTUDIO_MODEL = ["qwen3-235b-a22b-instruct-2507-mlx@4bit", "microsoft/phi-4-reasoning-plus", "llama-3.3-70b-instruct@q4_k_m", "medgemma-27b-it", "mistralai/mistral-small-3.2", "google/gemma-3-12b"]

# model_options = ["--Select--"] + OLLAMA_MODEL + OPENAI_MODEL
model_options = ["--Select--"] + OLLAMA_MODEL + LMSTUDIO_MODEL + OPENAI_MODEL
...
if not output_type == "--Select--" and output_agent_setting:
    ...
    multi_agent_prompts[0] = st.text_area(
        "Prompt Editing – Scale 1 Agent", value=AGENT_PROMPT_1, height=200
    )
    multi_agent_prompts[1] = st.text_area(
        "Prompt Editing – Scale 2 Agent", value=AGENT_PROMPT_2, height=200
    )
    multi_agent_prompts[2] = st.text_area(
        "Prompt Editing – Scale 3 Agent", value=AGENT_PROMPT_3, height=200
    )
    multi_agent_prompts[3] = st.text_area(
        "Prompt Editing – Scale 4 Agent", value=AGENT_PROMPT_4, height=200
    )
    multi_agent_prompts[4] = st.text_area(
        "Prompt Editing – Scale 5 Agent", value=AGENT_PROMPT_5, height=200
    )
    multi_agent_prompts[5] = st.text_area(
        "Prompt Editing – Scale 6 Agent", value=AGENT_PROMPT_6, height=200
    )
    multi_agent_prompts[6] = st.text_area(
        "Prompt Editing – Scale 7 Agent", value=AGENT_PROMPT_7, height=200
    )
    system_prompt_paragraph_multi = st.text_area(
        "Prompt Editing – Paragraph Agent",
        value=SYSTEM_PROMPT_PARAGRAPH_MULTI,
        height=200,
    )
...
button_disabled = (
    model == "--Select--"
    or output_type == "--Select--"
    or output_agent_style == "--Select--"
    or not resident_text.strip()
    or not attending_text.strip()
)
...
if st.button("Analyze", disabled=button_disabled):
    ...
    if output_agent_setting:
        # ... inside the "Analyze" button handler, single-agent branch
        system_prompt = edited_prompt
        if model in LMSTUDIO_MODEL:
            maybe_coro = extract_LMStudio(system_prompt, text, model)
            response = (
        ...
        # system_prompt = edited_prompt
        # if model in LMSTUDIO_MODEL:
        #    response = extract_LMStudio(system_prompt,text,model)
        # else:
        #    response = asyncio.run(extract_OpenAI(system_prompt,text,model))
        ...
        if output_setting:
            ...
            if model_type:
                ...
                first_part = asyncio.run(
                    extract_OpenAI(system_prompt_paragraph_multi, text, model)
                )
            ...
            for _, row in multi_df.iterrows():
                ...
                second_part += (
                    f'\n\n{row["Difference Type"]}: "{row["Resident Report"]}" --> '
                    f'"{row["Attending Report"]}"\n\n{row["Explanation"]}'
                )
